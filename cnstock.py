__author__ = 'rocchen'
# python 获取 中国证券网 的公告 working v1.0 http://30daydo.com/article/59
from bs4 import BeautifulSoup
from urllib import request
from urllib.error import URLError,HTTPError
import datetime, time, codecs, random, threading
import os,sys
...
def getInfo(max_index_user=5):
    ...
    while index < max_index:
        ...
        req = request.Request(url=company_news_site, headers=headers)
        resp = None
        raw_content = ""
        try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <2:
        n = input("Input Period : ? mins to download every cycle")
    else:
        n=int(sys.argv[1])
    execute_task(n)
    end_time = time.time()
